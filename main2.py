...
# Rest of the code...
# Connect to MongoDB
mongo_client = MongoClient("mongodb+srv://aakash:aakashsat1999*@cluster0.pue5wip.mongodb.net/")
...
# Create a SQLAlchemy engine to connect to the SQL data warehouse
###############################################################################################################
engine = sa.create_engine('mysql+pymysql://aakash:aakashsat1999*@localhost:3306/test')
###############################################################################################################
# Create a session factory
Session = sessionmaker(bind=engine)

API_KEY = "AIzaSyC9ATDinGhk_MAkD0m8chOgD1oeOCAPyhM"
...
def query_data(channel_name):
    session = Session()

    # Join the Channel and Video tables using a SQL JOIN query
    query = session.query(Channel, Video).join(Video, Channel.id == Video.channel_id).filter(Channel.name == channel_name)

    # Execute the query and fetch the results
    results = query.all()

    # Process and display the results
    for channel, video in results:
        st.write(f"Channel: {channel.name}")
...
def execute_query(query):
    try:
        result = mysql_cursor.execute(text(query))
        results = mysql_cursor.fetchall()
        if len(results) > 0:
